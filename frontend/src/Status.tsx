...
const SUPERIOR = [
  ...
  '營運指揮新展示層:指揮中心(待人雙佇列+AI 主理人動態+預警+營收帶)/ 經營總覽(月營收趨勢/客戶貢獻/續約雷達/團隊負載)/ 客戶 360 / 發佈行事曆(與既有頁並存供對比)',
  '營收認列引擎:刊例價+合約金額 → 結案認列 / 在途 / 洽談 pipeline 金額(GET /board/revenue)',
  ...
  '自動化測試 pytest 169 通過 / 1 skip(含合成夾具斷言圖片排列位置;skip 需 OCR extra)+ 前端 vitest 75',
]
const PARITY = [
...
const UNIT_TESTS: Row[] = [
  ['後端自動化測試 pytest', 'uv run pytest', '169 通過 / 1 skip(OCR extra)', true],
  ['前端自動化測試 vitest', 'pnpm test(jsdom+RTL)', '75 通過(路由/子導覽/看板三視圖/上傳/保真守門/XSS/進度估算/safeUrl/廣告版位地圖/檔期解析/狀態看板)', true],
  ...
  return (
    <div className="panel">
      ...
      <p className="hint" style={{ marginTop: 0 }}>
        ...
        <b>第一輪</b>發現多模組缺單元測試 → <b>分輪補測至 pytest 48→169 + 前端 75 vitest</b> → 稽核分全面回升至 78–88。
...
function StatCards() {
  ...
  return (
    <div className="stat-cards">
      ...
      <Stat label="後端測試" value="169 ✓" ok />
      <Stat label="前端測試" value="75 ✓" ok />
    </div>
  )
}
  ...
  return (
    <div className="status-page">
      ...
      <div className="panel">
        ...
        <p className="hint">後端 <code>uv run pytest</code> 169 通過 / 1 skip;前端 <code>pnpm test</code> 75 通過,並以隔離瀏覽器經 tunnel 實測。</p>
      </div>
    </div>
  )
}
...
const API_GROUPS: { group: string; rows: [string, string, string][] }[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      ['GET', '/board/revenue', '經營總覽:月營收趨勢 / 本月/季/YTD / 在途 / 洽談 pipeline / 客戶貢獻 / 續約雷達 / 團隊負載'],
    ],
  },
  {
    group: '廣告版位 / 自動化',
...
# 全套測試
cd backend && uv run pytest -q          # 後端 169 通過 / 1 skip
cd frontend && pnpm test -- --run       # 前端 75 通過
