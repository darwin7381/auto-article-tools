...
const SUPERIOR = [
  ...
  '自動化測試 pytest 139 通過 / 1 skip(含合成夾具斷言圖片排列位置;skip 需 OCR extra)+ 前端 vitest 66',
]
const PARITY = [
...
const UNIT_TESTS: Row[] = [
  ...
  ['前端自動化測試 vitest', 'pnpm test(jsdom+RTL)', '66 通過(路由/子導覽/看板三視圖/上傳/保真守門/XSS/進度估算/safeUrl/廣告版位地圖/檔期解析/狀態看板)', true],
  ...
  return (
    <div className="panel">
      ...
      <p className="hint" style={{ marginTop: 0 }}>
        ...
        <b>第一輪</b>發現多模組缺單元測試 → <b>分輪補測至 pytest 48→139 + 前端 66 vitest</b> → 稽核分全面回升至 78–88。
  ...
  return (
    <div className="status-page">
      ...
      <div className="panel">
        ...
        <p className="hint">後端 <code>uv run pytest</code> 139 通過 / 1 skip;前端 <code>pnpm test</code> 66 通過,並以隔離瀏覽器經 tunnel 實測。</p>
      </div>
    </div>
  )
}
...
cd frontend && pnpm test -- --run       # 前端 66 通過
