...
interface AiProcessingStageProcessorProps {
  // 從提取階段獲取的數據
 ...
 * 負責處理AI內容增強階段邏輯
 */
export default function AiProcessingStageProcessor({
  extractResult,
  onProcessComplete,
  onProcessError
}: AiProcessingStageProcessorProps) {
  const { 
    updateStageProgress, 
    completeStage, 
    moveToNextStage,
  ...
  const startAiProcessing = async () => {
    if (!extractResult.markdownKey) {
      onProcessError('缺少必要的Markdown數據', extractResult);
      return;
    }
