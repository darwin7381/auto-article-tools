...
import { getFileFromR2 } from '../storage/r2Service';
import { saveMarkdown } from '../document/markdownService';
...
/**
 * 處理Markdown內容，使用OpenAI進行增強
 * @param markdownContent Markdown內容
 * @returns 處理後的內容
 */
export async function processContentWithOpenAI(markdownContent: string): Promise<string> {
  if (!openaiClient) {
...
${markdownContent}`;
  ...
  try {
    ...
    if (!content) {
      console.warn('OpenAI回應為空，返回原始內容');
      return markdownContent;
    }

    console.log('OpenAI處理成功');
    return content;
  } catch (error) {
...
/**
 * 從R2獲取Markdown並使用OpenAI進行處理
 * @param fileId 文件ID
 * @param markdownKey Markdown文件在R2的鍵值
 * @returns 處理結果
 */
export async function enhanceMarkdownWithOpenAI(fileId: string, markdownKey: string): Promise<{
  success: boolean;
  fileId: string;
  markdownKey: string;
  markdownUrl: string;
  ...
  try {
    ...
    try {
      // 使用OpenAI處理內容
      const enhancedContent = await processContentWithOpenAI(markdownContent);
...
${enhancedContent}`;
      ...
      console.error('OpenAI處理異常:', aiError);
      
      // 如果OpenAI處理失敗，仍然保存原始內容
      const finalMarkdown = `---
...
${markdownContent}`;
    ...
    console.error('OpenAI整體處理失敗:', error);
    throw error;
  }
} 