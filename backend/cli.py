...
async def _eval(file: str | None, url: str | None, judge: bool = False) -> None:
    ...
    result = None
    stages: list[dict] = []
    source_md = ""  # 抽取後原文,供 LLM-judge 比對忠實度
    ...
    if judge:
        from app.services.evals import llm_judge

        wp = (result or {}).get("wordpress") or {}
        print("\n=== LLM-judge 品質評審(比對原文 vs 成稿) ===")
        try:
        ...
        if j.missing_content:
            print("  ⚠️ 成稿遺失(原文有):")
            for x in j.missing_content:
                print(f"     - {x}")
        if j.hallucinations:
            print("  ⚠️ 疑似幻覺(原文無):")
            for x in j.hallucinations:
                print(f"     - {x}")
        if j.issues:
...
def main() -> None:
    ...
    p.add_argument("--judge", action="store_true", help="eval:加跑 LLM-judge 品質評審(比對原文 vs 成稿)")
